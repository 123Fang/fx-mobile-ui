...
import { Search } from 'ctf-ics-mobile-ui';
...
```html
<icsm-search v-model="value" placeholder="请输入搜索关键词" />
```
...
> Tips: 在 icsm-search 外层增加 form 标签，且 action 不为空，即可在 iOS 输入法中显示搜索按钮。
...
```html
<icsm-search v-model="value" disabled placeholder="请输入搜索关键词" />
```
...
| id | 搜索框 id，同时会设置 label 的 for 属性 | _string_ | `icsm-search-n-input` |
...
```ts
import type { SearchProps, SearchShape, SearchInstance } from 'ctf-ics-mobile-ui';
```
...
```ts
import { ref } from 'vue';
import type { SearchInstance } from 'ctf-ics-mobile-ui';
...
| 名称                            | 默认值                       | 描述 |
| ------------------------------- | ---------------------------- | ---- |
| --icsm-search-padding            | _10px var(--icsm-padding-sm)_ | -    |
| --icsm-search-background         | _var(--icsm-background-2)_    | -    |
| --icsm-search-content-background | _var(--icsm-gray-1)_          | -    |
| --icsm-search-input-height       | _34px_                       | -    |
| --icsm-search-label-padding      | _0 5px_                      | -    |
| --icsm-search-label-color        | _var(--icsm-text-color)_      | -    |
| --icsm-search-label-font-size    | _var(--icsm-font-size-md)_    | -    |
| --icsm-search-left-icon-color    | _var(--icsm-gray-6)_          | -    |
| --icsm-search-action-padding     | _0 var(--icsm-padding-xs)_    | -    |
| --icsm-search-action-text-color  | _var(--icsm-text-color)_      | -    |
| --icsm-search-action-font-size   | _var(--icsm-font-size-md)_    | -    |
