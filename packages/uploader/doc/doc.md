...
import { Uploader } from 'fx-mobile-ui';
...
```html
<fx-uploader v-model="fileList" multiple />
```
...
```html
<fx-uploader v-model="fileList" :after-read="afterRead" />
```
...
```html
<fx-uploader v-model="fileList" multiple :max-count="2" />
```
...
```html
<fx-uploader multiple :max-size="500 * 1024" @oversize="onOversize" />
```
...
```html
<fx-uploader multiple :max-size="isOverSize" />
```
...
```html
<fx-uploader>
  <fx-button icon="plus" type="primary">上传文件</fx-button>
</fx-uploader>
```
...
```html
<!-- 不指定单位，默认为 px -->
<fx-uploader v-model="fileList" preview-size="60" />
<!-- 指定单位，支持 rem, vh, vw -->
<fx-uploader v-model="fileList" preview-size="5rem" />
```

将 `preview-size` 设置为数组格式，可以分别设置宽高。数组第一项对应宽度，数组第二项对应高度。

```html
<fx-uploader v-model="fileList" :preview-size="[60, 40]" />
```
...
```html
<fx-uploader :before-read="beforeRead" />
```
...
```html
<fx-uploader v-model="fileList" :deletable="false" />
```
...
```html
<fx-uploader v-model="fileList" reupload max-count="2" />
```
...
```ts
import { ref } from 'vue';
import type { UploaderInstance } from 'fx-mobile-ui';
...
| 名称 | 默认值 | 描述 |
| --- | --- | --- |
| --fx-uploader-size | _80px_ | - |
| --fx-uploader-icon-size | _24px_ | - |
| --fx-uploader-icon-color | _var(--fx-gray-4)_ | - |
| --fx-uploader-text-color | _var(--fx-text-color-2)_ | - |
| --fx-uploader-text-font-size | _var(--fx-font-size-sm)_ | - |
| --fx-uploader-upload-background | _var(--fx-gray-1)_ | - |
| --fx-uploader-upload-active-color | _var(--fx-active-color)_ | - |
| --fx-uploader-delete-color | _var(--fx-white)_ | - |
| --fx-uploader-delete-icon-size | _14px_ | - |
| --fx-uploader-delete-background | _rgba(0, 0, 0, 0.7)_ | - |
| --fx-uploader-file-background | _var(--fx-background)_ | - |
| --fx-uploader-file-icon-size | _20px_ | - |
| --fx-uploader-file-icon-color | _var(--fx-gray-7)_ | - |
| --fx-uploader-file-name-padding | _0 var(--fx-padding-base)_ | - |
| --fx-uploader-file-name-margin-top | _var(--fx-padding-xs)_ | - |
| --fx-uploader-file-name-font-size | _var(--fx-font-size-sm)_ | - |
| --fx-uploader-file-name-text-color | _var(--fx-gray-7)_ | - |
| --fx-uploader-mask-text-color | _var(--fx-white)_ | - |
| --fx-uploader-mask-background | _fade(var(--fx-gray-8), 88%)_ | - |
| --fx-uploader-mask-icon-size | _22px_ | - |
| --fx-uploader-mask-message-font-size | _var(--fx-font-size-sm)_ | - |
| --fx-uploader-mask-message-line-height | _var(--fx-line-height-xs)_ | - |
| --fx-uploader-loading-icon-size | _22px_ | - |
| --fx-uploader-loading-icon-color | _var(--fx-white)_ | - |
| --fx-uploader-disabled-opacity | _var(--fx-disabled-opacity)_ | - |
| --fx-uploader-border-radius | _0px_ | - |
