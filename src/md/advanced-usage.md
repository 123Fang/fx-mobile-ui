...
// 方式一. 通过 app.use 注册
// 注册完成后，在模板中通过 <icsm-button> 或 <VanButton> 标签来使用按钮组件
app.use(Button);

// 方式二. 通过 app.component 注册
// 注册完成后，在模板中通过 <icsm-button> 标签来使用按钮组件
...
```js
import Vant from 'ctf-ics-mobile-ui';
import { createApp } from 'vue';
...
<icsm-checkbox v-model="checked" ref="checkbox"> 复选框 </icsm-checkbox>
```
...
<!-- 开启顶部安全区适配 -->
<icsm-nav-bar safe-area-inset-top />

<!-- 开启底部安全区适配 -->
<icsm-number-keyboard safe-area-inset-bottom />
```
