...
// 方式一. 通过 app.use 注册
// 注册完成后，在模板中通过 <fx-button> 或 <VanButton> 标签来使用按钮组件
app.use(Button);

// 方式二. 通过 app.component 注册
// 注册完成后，在模板中通过 <fx-button> 标签来使用按钮组件
...
<fx-checkbox v-model="checked" ref="checkbox"> 复选框</fx-checkbox>
```
...
module.exports = {
  plugins: {
    ...
    'postcss-pxtorem': {
      rootValue({ file }) {
        return file.indexOf('ctf-ics-mobile-ui') !== -1 ? 37.5 : 75;
      },
      propList: ['*'],
    },
  },
...
<!-- 开启顶部安全区适配 -->
<fx-nav-bar safe-area-inset-top />

<!-- 开启底部安全区适配 -->
<fx-number-keyboard safe-area-inset-bottom />
```

<img  style='width: 100%' src="https://fastly.jsdelivr.net/npm/@vant/assets/safearea.png">
